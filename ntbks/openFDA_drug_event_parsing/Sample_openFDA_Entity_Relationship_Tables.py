...
report_er_df_to_save = (
    report_er_df.groupby(primarykey).agg(max).reset_index().dropna(subset=[primarykey])
)
report_er_df_to_save.to_csv(er_dir + "report.csv.gz", compression="gzip", index=False)
try:
    del report_er_df_to_save
except NameError:
    pass
...
report_serious_er_df = (
    report_df[columns]  # Uses report_df
    .rename(columns=rename_columns)
    .set_index(primarykey)
    .sort_index()
    .reset_index()
...
report_serious_er_df = report_serious_er_df.reindex(
    np.sort(report_serious_er_df.columns), axis=1
)
...
(report_serious_er_df).to_csv(
    er_dir + "report_serious.csv.gz", compression="gzip", index=False
)
...
# Clean up DataFrames from the 'report' section
try:
    del report_df
except NameError:
    pass
try:
    del report_er_df  # Should have been deleted after creating report.csv.gz if not used by report_er_df_to_save
except NameError:
    pass
try:
...
try:
    ...
    pass
try:
    del df  # Generic df from loop
except NameError:
    pass
try:
    del results  # Generic results from loop
except NameError:
    pass
...
# Clean up DataFrames from the 'patient' section
try:
    del patient_df
except NameError:
    pass
...
drugcharacteristics_er_df = drugcharacteristics_er_df.reindex(
    np.sort(drugcharacteristics_er_df.columns), axis=1
)
...
(drugcharacteristics_er_df).to_csv(
    er_dir + "drugcharacteristics.csv.gz", compression="gzip", index=False
)
...
# Clean up DataFrames from the 'drug_characteristics' section
try:
    del patient_drug_df
except NameError:
    pass
...
patient_drug_openfda_rxcui_df[primarykey] = patient_drug_openfda_rxcui_df[
    primarykey
].astype(str)
...
drugs_er_df = (
    patient_drug_openfda_rxcui_df[columns]
    .rename(columns=rename_columns)
    .set_index(primarykey)
    .sort_index()
    .reset_index()
...
# Clean up DataFrames from the 'drugs' section
try:
...
# Clean up DataFrames from the 'reactions' section
try:
...
# These are placeholder paths. You need to download the OMOP Vocabulary files (e.g., from OHDSI Athena)
# and place them in a directory accessible by this script.
concept_file_path = "../../data/vocabulary_download_v5/CONCEPT.csv"  # Example path
concept_relationship_file_path = (
    "../../data/vocabulary_download_v5/CONCEPT_RELATIONSHIP.csv"  # Example path
)

try:
    concept = pd.read_csv(
        concept_file_path,  # Use the variable
        sep="\\t",
        dtype={"concept_id": "int"},
        on_bad_lines="skip",  # Added to handle potential issues in sample/real files
    )
    ...
    print(
        "Please download OHDSI vocabulary files (e.g., from Athena) and update the path in the script."
    )
    concept = (
        pd.DataFrame()
    )  # Create an empty DataFrame to avoid subsequent errors if file not found
except Exception as e:
    print(f"Error loading {concept_file_path}: {e}")
...
try:
    concept_relationship = pd.read_csv(
        concept_relationship_file_path,  # Use the variable
        sep="\\t",
        dtype={"concept_id_1": "int", "concept_id_2": "int"},
        on_bad_lines="skip",  # Added to handle potential issues
    )
    print("CONCEPT_RELATIONSHIP.csv loaded successfully.")
    concept_relationship.head()
except FileNotFoundError:
    print(f"VOCABULARY FILE NOT FOUND: {concept_relationship_file_path}")
    print(
        "Please download OHDSI vocabulary files (e.g., from Athena) and update the path in the script."
    )
    concept_relationship = pd.DataFrame()  # Create an empty DataFrame
except Exception as e:
    print(f"Error loading {concept_relationship_file_path}: {e}")
...
drugs = pd.read_csv(
    er_dir + "drugs.csv.gz", compression="gzip", dtype={"safetyreportid": "str"}
)
...
rxnorm_concept_ids = (
    rxnorm_concept.query("concept_code in @intersect")["concept_id"]
    .astype(int)
    .unique()
)
...
pickle.dump(ids, open("../../data_SAMPLE/all_openFDA_rxnorm_concept_ids.pkl", "wb"))
...
patient_reaction_df = pd.read_csv(
    er_dir + "reactions.csv.gz", compression="gzip", dtype={"safetyreportid": "str"}
)
...
patient_reaction_df["reaction_meddrapt"] = (
    patient_reaction_df["reaction_meddrapt"].astype(str).str.title()
)
meddra_concept["concept_name"] = meddra_concept["concept_name"].astype(str).str.title()
print(patient_reaction_df.shape[0])
joined = (
    (
        patient_reaction_df.set_index("reaction_meddrapt")
        .join(
            meddra_concept.query('concept_class_id=="PT"')
    ...
    .rename_axis("MedDRA_concept_name")
    .reset_index()
)
joined = joined.reindex(np.sort(joined.columns), axis=1)
...
print(
    len(np.intersect1d(all_reports, joined_notnull.safetyreportid.astype(str).unique()))
    / len(all_reports)
)
...
pickle.dump(
    ...
    open("../../data_SAMPLE/all_openFDA_meddra_concept_ids.pkl", "wb"),
)
...
(
    joined_notnull.to_csv(
        er_dir + "standard_reactions.csv.gz", compression="gzip", index=False
    )
)
...
rxnorm_to_atc_relationships = (
    r.query("concept_id_1 in @openfda_concept_ids & concept_id_2 in @atc_concept_ids")
...
rxnorm_to_atc_relationships.RxNorm_concept_id = (
    rxnorm_to_atc_relationships.RxNorm_concept_id.astype(int)
)
rxnorm_to_atc_relationships.ATC_concept_id = (
    rxnorm_to_atc_relationships.ATC_concept_id.astype(int)
)

rxnorm_to_atc_relationships = rxnorm_to_atc_relationships.reindex(
    np.sort(rxnorm_to_atc_relationships.columns), axis=1
)
...
standard_drugs_atc = (
    standard_drugs.loc[:, ["RxNorm_concept_id", "safetyreportid"]]
    .drop_duplicates()
...
standard_drugs_atc = standard_drugs_atc.reindex(
    np.sort(standard_drugs_atc.columns), axis=1
)
standard_drugs_atc.ATC_concept_id = standard_drugs_atc.ATC_concept_id.astype(int)
print(
    len(np.intersect1d(all_reports, standard_drugs_atc.safetyreportid.unique()))
    / len(all_reports)
)
...
standard_drugs_atc.to_csv(
    er_dir + "standard_drugs_atc.csv.gz", compression="gzip", index=False
)
...
all_openFDA_rxnorm_concept_ids = pickle.load(
    open("../../data_SAMPLE/all_openFDA_rxnorm_concept_ids.pkl", "rb")
)
...
all_rxnorm_concept_ids = (
    concept.query('vocabulary_id=="RxNorm"').concept_id.astype(int).unique()
)
...
r = (
    concept_relationship.loc[:, ["concept_id_1", "concept_id_2", "relationship_id"]]
    .drop_duplicates()
    .dropna()
    .copy()
)
...
rxnorm_relationships = r.query(
    "concept_id_1 in @all_rxnorm_concept_ids & "
...
first_second_relations = (
    r.query("concept_id_1 in @all_openFDA_rxnorm_concept_ids")
    .set_index("concept_id_1")
...
first_second_relations = first_second_relations[
    first_second_relations.RxNorm_concept_id_1
    != first_second_relations.RxNorm_concept_id_2
]
print(first_second_relations.shape)
first_second_relations = first_second_relations.reindex(
    np.sort(first_second_relations.columns), axis=1
)
...
second_third_relations = (
    r.query("concept_id_1 in @ids")
    .set_index("concept_id_1")
    .join(c.set_index("concept_id"))
    .rename(
        columns={
...
second_third_relations = second_third_relations[
    second_third_relations.RxNorm_concept_id_2
    != second_third_relations.RxNorm_concept_id_3
]
print(second_third_relations.shape)
second_third_relations = second_third_relations.reindex(
    np.sort(second_third_relations.columns), axis=1
)
...
third_fourth_relations = third_fourth_relations[
    third_fourth_relations.RxNorm_concept_id_3
    != third_fourth_relations.RxNorm_concept_id_4
]
print(third_fourth_relations.shape)
third_fourth_relations = third_fourth_relations.reindex(
    np.sort(third_fourth_relations.columns), axis=1
)
...
fourth_fifth_relations = fourth_fifth_relations[
    fourth_fifth_relations.RxNorm_concept_id_4
    != fourth_fifth_relations.RxNorm_concept_id_5
]
print(fourth_fifth_relations.shape)
fourth_fifth_relations = fourth_fifth_relations.reindex(
    np.sort(fourth_fifth_relations.columns), axis=1
)
...
fifth_sixth_relations = fifth_sixth_relations[
    fifth_sixth_relations.RxNorm_concept_id_5
    != fifth_sixth_relations.RxNorm_concept_id_6
]
print(fifth_sixth_relations.shape)
fifth_sixth_relations = fifth_sixth_relations.reindex(
    np.sort(fifth_sixth_relations.columns), axis=1
)
...
rxnorm_to_ings123 = (
    first_second_relations.set_index(
        [
            "RxNorm_concept_id_2",
            "RxNorm_concept_code_2",
            "RxNorm_concept_name_2",
            "RxNorm_concept_class_id_2",
        ]
    )
    .join(
        second_third_relations.set_index(
            [
                "RxNorm_concept_id_2",
                "RxNorm_concept_code_2",
                "RxNorm_concept_name_2",
    ...
    .query(
        'RxNorm_concept_class_id_3=="Ingredient" & '
...
(
    rxnorm_to_ings123.loc[
        :, ["RxNorm_concept_name_1", "RxNorm_concept_name_3"]
    ].drop_duplicates()
).head()
...
rxnorm_to_ings1234 = (
    ...
    .join(
        third_fourth_relations.set_index(
            [
                "RxNorm_concept_id_3",
                "RxNorm_concept_code_3",
                "RxNorm_concept_name_3",
...
rxnorm_to_ings1234 = rxnorm_to_ings1234.reindex(
    np.sort(rxnorm_to_ings1234.columns), axis=1
)
...
(
    rxnorm_to_ings1234.loc[
        :, ["RxNorm_concept_name_1", "RxNorm_concept_name_4"]
    ].drop_duplicates()
).head()
len(
    np.intersect1d(
...
rxnorm_to_ings12345 = (
    ...
    .query(
        'RxNorm_concept_class_id_4!="Ingredient" & '
    ...
    .join(
        fourth_fifth_relations.set_index(
            [
                "RxNorm_concept_id_4",
                "RxNorm_concept_code_4",
                "RxNorm_concept_name_4",
...
rxnorm_to_ings12345 = rxnorm_to_ings12345.reindex(
    np.sort(rxnorm_to_ings12345.columns), axis=1
)
...
(
    rxnorm_to_ings12345.loc[
        :, ["RxNorm_concept_name_1", "RxNorm_concept_name_5"]
    ].drop_duplicates()
).head()
len(
    np.intersect1d(
...
rxnorm_to_ings123456 = (
    ...
    .query(
        'RxNorm_concept_class_id_5!="Ingredient" & '
    ...
    .join(
        fifth_sixth_relations.set_index(
            [
                "RxNorm_concept_id_5",
                "RxNorm_concept_code_5",
                "RxNorm_concept_name_5",
...
rxnorm_to_ings123456 = rxnorm_to_ings123456.reindex(
    np.sort(rxnorm_to_ings123456.columns), axis=1
)
...
(
    rxnorm_to_ings123456.loc[
        :, ["RxNorm_concept_name_1", "RxNorm_concept_name_6"]
    ].drop_duplicates()
).head()
len(
    np.intersect1d(
...
rxnorm_to_ings123456_to_add = (
    rxnorm_to_ings123456.loc[
        ...
        [
            ...
            "RxNorm_concept_class_id_1",
            "RxNorm_concept_id_6",
            "RxNorm_concept_code_6",
            "RxNorm_concept_name_6",
            "RxNorm_concept_class_id_6",
        ],
    ]
    .drop_duplicates()
    ...
    np.intersect1d(
        np.union1d(
            np.union1d(
                np.union1d(
                    rxnorm_to_ings123.RxNorm_concept_id_1.dropna().astype(int).unique(),
                    rxnorm_to_ings1234.RxNorm_concept_id_1.dropna()
                    .astype(int)
                    .unique(),
                ),
                rxnorm_to_ings12345.RxNorm_concept_id_1.dropna().astype(int).unique(),
...
rxnorm_to_ings_all.RxNorm_concept_id_2 = rxnorm_to_ings_all.RxNorm_concept_id_2.astype(
    int
)
...
len(
    np.intersect1d(
        rxnorm_to_ings_all.RxNorm_concept_id_1, all_openFDA_rxnorm_concept_ids
    )
) / len(all_openFDA_rxnorm_concept_ids)
...
standard_drug_ingredients = (
    (
        standard_drug.loc[:, ["RxNorm_concept_id", "safetyreportid"]]
        .drop_duplicates()
        .set_index(["RxNorm_concept_id"])
    )
    .join(
        rxnorm_to_ings_all.loc[
            :,
            [
                "RxNorm_concept_id_1",
                "RxNorm_concept_id_2",
...
standard_drug_ingredients = standard_drug_ingredients.reindex(
    np.sort(standard_drug_ingredients.columns), axis=1
)
...
print(
    len(
        np.intersect1d(
            standard_drug_ingredients.safetyreportid.astype(str).unique(), all_reports
        )
    )
    / len(all_reports)
)
...
(
    standard_drug_ingredients.to_csv(
        er_dir + "standard_drugs_rxnorm_ingredients.csv.gz",
        compression="gzip",
        index=False,
    )
)
...
standard_reactions = pd.read_csv(
    er_dir + "standard_reactions.csv.gz",
    compression="gzip",
    dtype={"safetyreportid": "str"},
)
...
(
    r.query(
        "concept_id_1 in @all_meddra_concept_ids & "
...
all_meddra_relationships = (
    r.query(
        "concept_id_1 in @meddra_concept_ids & " + "concept_id_2 in @meddra_concept_ids"
    )
    .set_index("concept_id_1")
    .join(
...
all_meddra_relationships = all_meddra_relationships.reindex(
    np.sort(all_meddra_relationships.columns), axis=1
)
...
all_meddra_relationships.MedDRA_concept_id_1 = (
    all_meddra_relationships.MedDRA_concept_id_1.astype(int)
)
all_meddra_relationships.MedDRA_concept_code_1 = (
    all_meddra_relationships.MedDRA_concept_code_1.astype(int)
)
all_meddra_relationships.MedDRA_concept_id_2 = (
    all_meddra_relationships.MedDRA_concept_id_2.astype(int)
)
all_meddra_relationships.MedDRA_concept_code_2 = (
    all_meddra_relationships.MedDRA_concept_code_2.astype(int)
)
...
first_rxs = reactions
first_relations = (
    all_meddra_relationships.query(
        "MedDRA_concept_id_1 in @first_rxs & " + 'MedDRA_concept_class_id_2=="HLT"'
    )
).reset_index(drop=True)
...
second_relations = (
    all_meddra_relationships.query(
        "MedDRA_concept_id_1 in @second_rxs & " + 'MedDRA_concept_class_id_2=="HLGT"'
    ).rename(
        columns={
...
third_relations = (
    all_meddra_relationships.query(
        "MedDRA_concept_id_1 in @third_rxs & " + 'MedDRA_concept_class_id_2=="SOC"'
    ).rename(
        columns={
...
first_second_third_relations = (
    first_relations.set_index("MedDRA_concept_id_2")
    .join(
        second_relations.loc[
            :,
            [
                "MedDRA_concept_id_2",
                "MedDRA_concept_id_3",
...
first_second_third_relations = first_second_third_relations.reindex(
    np.sort(first_second_third_relations.columns), axis=1
)
first_second_third_relations["MedDRA_concept_id_3"] = first_second_third_relations[
    "MedDRA_concept_id_3"
].astype(int)
...
first_second_third_fourth_relations = first_second_third_fourth_relations.reindex(
    np.sort(first_second_third_fourth_relations.columns), axis=1
)
...
len(
    np.setdiff1d(
        reactions, first_second_third_fourth_relations.MedDRA_concept_id_1.unique()
    )
)
...
df1 = (
    standard_reactions.loc[:, ["MedDRA_concept_id"]]
    .drop_duplicates()
    .dropna()
...
df2 = first_second_third_fourth_relations.set_index("MedDRA_concept_id_1")
print(df2.shape)
...
standard_reactions_pt_to_hlt = (
    standard_reactions.loc[:, ["safetyreportid", "MedDRA_concept_id"]]
    .drop_duplicates()
    .set_index(["MedDRA_concept_id"])
    .join(
        pt_to_hlt.loc[
...
standard_reactions_pt_to_hlt = standard_reactions_pt_to_hlt.reindex(
    np.sort(standard_reactions_pt_to_hlt.columns), axis=1
)
...
(
    standard_reactions_pt_to_hlt.to_csv(
        er_dir + "standard_reactions_meddra_hlt.csv.gz", compression="gzip", index=False
    )
)
...
standard_reactions_pt_to_hlgt = standard_reactions_pt_to_hlgt.reindex(
    np.sort(standard_reactions_pt_to_hlgt.columns), axis=1
)
...
(
    standard_reactions_pt_to_hlgt.to_csv(
        er_dir + "standard_reactions_meddra_hlgt.csv.gz",
        compression="gzip",
        index=False,
    )
)
...
standard_reactions_pt_to_soc = standard_reactions_pt_to_soc.reindex(
    np.sort(standard_reactions_pt_to_soc.columns), axis=1
)
...
(
    standard_reactions_pt_to_soc.to_csv(
        er_dir + "standard_reactions_meddra_soc.csv.gz", compression="gzip", index=False
    )
)
...
m_to_s_r = (
    ...
    .set_index("concept_id_2")
    .join(
        concept.query('vocabulary_id=="SNOMED"')
        .loc[:, ["concept_id", "concept_code", "concept_class_id", "concept_name"]]
        .drop_duplicates()
        .set_index("concept_id")
    )
...
pts = standard_reactions_meddra_relationships.query(
    'MedDRA_concept_class_id_1=="PT"'
).MedDRA_concept_id_1.unique()
print(len(np.intersect1d(pts, r2s)) / len(pts))
print(len(np.intersect1d(pts, r2s)) / len(r2s))

df = standard_reactions_meddra_relationships.query("MedDRA_concept_id_1 in @r2s")

print(df.shape)

joinedpt = (
    df.set_index("MedDRA_concept_id_1")
    .join(m_to_s_r.query("MedDRA_concept_id in @pts").set_index("MedDRA_concept_id"))
    .rename_axis("MedDRA_concept_id_1")
    .reset_index()
    .rename(
        columns={
...
hlts = joinedpt.query('MedDRA_concept_class_id_2=="HLT"').MedDRA_concept_id_2.unique()
...
joinedhlt = (
    df.set_index("MedDRA_concept_id_2")
    .join(m_to_s_r.query("MedDRA_concept_id in @hlts").set_index("MedDRA_concept_id"))
    .rename_axis("MedDRA_concept_id_2")
    .reset_index()
    .rename(
        columns={
...
hlgts = joinedhlt.query(
    'MedDRA_concept_class_id_3=="HLGT"'
).MedDRA_concept_id_3.unique()
...
joinedhlgt = (
    df.set_index("MedDRA_concept_id_3")
    .join(m_to_s_r.query("MedDRA_concept_id in @hlgts").set_index("MedDRA_concept_id"))
...
socs = joinedhlgt.query('MedDRA_concept_class_id_4=="SOC"').MedDRA_concept_id_4.unique()
...
joinedsoc = (
    df.set_index("MedDRA_concept_id_4")
    .join(m_to_s_r.query("MedDRA_concept_id in @socs").set_index("MedDRA_concept_id"))
...
allmeddraconcepts = standard_reactions_meddra_relationships.query(
    'MedDRA_concept_class_id_1=="PT"'
).MedDRA_concept_id_1.unique()
...
print(len(smeddraconcepts))
allmeddraconcepts = standard_reactions_meddra_relationships.query(
    'MedDRA_concept_class_id_4=="SOC"'
).MedDRA_concept_id_4.unique()
print(len(allmeddraconcepts))
...
standard_reactions_meddrapt_to_snomed = (
    joinedsoc.loc[
        ...
        [
            "MedDRA_concept_id_1",
            "SNOMED_concept_id_1",
            "SNOMED_concept_code_1",
            "SNOMED_concept_name_1",
    ...
    .set_index("MedDRA_concept_id_1")
    .join(standard_reactions.drop_duplicates().set_index("MedDRA_concept_id"))
    .reset_index(drop=True)
    .drop(
        ["MedDRA_concept_code", "MedDRA_concept_name", "MedDRA_concept_class_id"],
        axis=1,
    )
    .dropna()
)
standard_reactions_meddrapt_to_snomed = standard_reactions_meddrapt_to_snomed.reindex(
    np.sort(standard_reactions_meddrapt_to_snomed.columns), axis=1
)
...
(
    standard_reactions_meddrapt_to_snomed.to_csv(
        er_dir + "standard_reactions_snomed.csv.gz", compression="gzip", index=False
    )
)
...
standard_reactions_meddrahlt_to_snomed = (
    joinedsoc.query('MedDRA_concept_class_id_2=="HLT"')
    .loc[
        :,
        [
            "MedDRA_concept_id_1",
            "SNOMED_concept_id_2",
            "SNOMED_concept_code_2",
            "SNOMED_concept_name_2",
...
standard_reactions_meddrahlt_to_snomed = standard_reactions_meddrahlt_to_snomed.reindex(
    np.sort(standard_reactions_meddrahlt_to_snomed.columns), axis=1
)
...
standard_reactions_meddrahlgt_to_snomed = (
    joinedsoc.query('MedDRA_concept_class_id_2=="HLGT"')
    .loc[
        :,
        [
            "MedDRA_concept_id_1",
            "SNOMED_concept_id_3",
            "SNOMED_concept_code_3",
            "SNOMED_concept_name_3",
            "SNOMED_concept_class_id_2",
        ],
    ]
    .drop_duplicates()
...
standard_reactions_meddrahlgt_to_snomed = (
    standard_reactions_meddrahlgt_to_snomed.reindex(
        np.sort(standard_reactions_meddrahlgt_to_snomed.columns), axis=1
    )
)
...
standard_reactions_meddrasoc_to_snomed = (
    joinedsoc.query('MedDRA_concept_class_id_4=="SOC"')
    .loc[
        :,
        [
            "MedDRA_concept_id_1",
            "SNOMED_concept_id_4",
            "SNOMED_concept_code_4",
            "SNOMED_concept_name_34",
...
standard_reactions_meddrasoc_to_snomed = standard_reactions_meddrasoc_to_snomed.reindex(
    np.sort(standard_reactions_meddrasoc_to_snomed.columns), axis=1
)
