...
patients = pd.read_csv(
    er_dir + "patient.csv.gz",
    compression="gzip",
    index_col=0,
    dtype={"safetyreportid": "str", "patient_custom_master_age": "float"},
)
...
term_newborn_infants = aged[age_col].apply(
    lambda x: float(x) > 0 and float(x) <= (1 / 12)
)
infants_and_toddlers = aged[age_col].apply(
    lambda x: float(x) > (1 / 12) and float(x) <= 2
)
...
neonates = aged[age_col].apply(lambda x: float(x) > 0 and float(x) < (1 / 12))
infants = aged[age_col].apply(lambda x: float(x) >= (1 / 12) and float(x) < 2)
children = aged[age_col].apply(lambda x: float(x) >= 2 and float(x) < 11)
...
report = pd.read_csv(
    er_dir + "report.csv.gz", compression="gzip", dtype={"safetyreportid": "str"}
)
report.head()
...
pediatric_patients_report = pd.merge(df1, df2, on=primarykey, how="inner").query(
    "safetyreportid in @ped_reports"
)
...
df1[primarykey] = df1[primarykey].astype(str)
df2[primarykey] = df2[primarykey].astype(str)
pediatric_patients_report_serious = pd.merge(df1, df2, on=primarykey, how="inner")
...
pediatric_patients_report_serious_reporter = pd.merge(
    df1, df2, on=primarykey, how="inner"
)
...
(
    pediatric_patients_report_serious_reporter.to_csv(
        "../../data/pediatric_patients_report_serious_reporter.csv.gz",
        compression="gzip",
    )
)
...
ped_reports = pediatric_patients_report_serious_reporter.safetyreportid.astype(
    str
).unique()
len(ped_reports)
...
pediatric_patients_report_serious_reporter = pd.read_csv(
    "../../data/pediatric_patients_report_serious_reporter.csv.gz",
    compression="gzip",
    index_col=0,
)
...
pediatric_standard_drugs_atc = pd.read_csv(
    "../../data/openFDA_drug_event/er_tables/standard_drugs_atc.csv.gz",
    compression="gzip",
    dtype={"safetyreportid": "str"},
).query("safetyreportid in @ped_reports")
pediatric_standard_drugs_atc.safetyreportid = (
    pediatric_standard_drugs_atc.safetyreportid.astype(str)
)
...
pediatric_standard_reactions = (
    pd.read_csv(er_dir + "standard_reactions.csv.gz", compression="gzip")
).query("safetyreportid in @ped_reports")
pediatric_standard_reactions.safetyreportid = (
    pediatric_standard_reactions.safetyreportid.astype(str)
)
pediatric_standard_reactions.MedDRA_concept_id = (
    pediatric_standard_reactions.MedDRA_concept_id.astype(int)
)
...
pediatric_patients_report_serious_reporter_drugs_reactions = (
    pediatric_patients_report_serious_reporter.set_index("safetyreportid")
    .join(pediatric_standard_drugs_atc.set_index("safetyreportid"))
    .dropna(subset=["ATC_concept_id"])
    .join(pediatric_standard_reactions.set_index("safetyreportid"))
    .dropna(subset=["MedDRA_concept_id"])
    .reset_index()
)
pediatric_patients_report_serious_reporter_drugs_reactions = (
    pediatric_patients_report_serious_reporter_drugs_reactions.reindex(
        np.sort(pediatric_patients_report_serious_reporter_drugs_reactions.columns),
        axis=1,
    )
)

pediatric_patients_report_serious_reporter_drugs_reactions.ATC_concept_id = (
    pediatric_patients_report_serious_reporter_drugs_reactions.ATC_concept_id.astype(
        int
    ).copy()
)

pediatric_patients_report_serious_reporter_drugs_reactions.MedDRA_concept_code = pediatric_patients_report_serious_reporter_drugs_reactions.MedDRA_concept_code.astype(
    int
).copy()

pediatric_patients_report_serious_reporter_drugs_reactions.MedDRA_concept_id = (
    pediatric_patients_report_serious_reporter_drugs_reactions.MedDRA_concept_id.astype(
        int
    ).copy()
)
...
print(
    pediatric_patients_report_serious_reporter_drugs_reactions.safetyreportid.nunique()
)
...
(
    pediatric_patients_report_serious_reporter_drugs_reactions.to_csv(
        "../../data/pediatric_patients_report_serious_reporter_drugs_reactions.csv.gz",
        compression="gzip",
    )
)
...
pediatric_standard_drugs = pd.read_csv(
    "../../data/openFDA_drug_event/er_tables/standard_drugs.csv.gz",
    compression="gzip",
    dtype={"safetyreportid": "str"},
).query("safetyreportid in @ped_reports")
pediatric_standard_drugs.safetyreportid = (
    pediatric_standard_drugs.safetyreportid.astype(str)
)
...
for rxfile in rxfiles:
    ...
    rxfile_dict[key] = pd.read_csv(
        "../../RxNorm_relationships_tables/" + rxfile, engine="c", index_col=0
    )
...
m = pd.merge(
    a, pd.concat(tobrand), left_on="RxNorm_concept_id", right_on="concept_id_1"
)
...
(
    m_renamed.to_csv(
        "../../data/pediatric_patients_report_drug_brands.csv.gz", compression="gzip"
    )
)
